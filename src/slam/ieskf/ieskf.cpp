...
namespace Slam
{
	...
	void IESKF::predict(const IMU& imu, double dt)// imu time
	{
		...
		Eigen::Matrix<double, 18, 18> Fx;//状态转移矩阵
		Eigen::Matrix<double, 18, 12> Fw;//控制输入矩阵
		Fw.setZero();
		Fx.setIdentity();
		//方差计算
	...
	bool IESKF::update()
	{
		...
		auto x_k_k = X;
		auto x_k_last = X;
		Eigen::MatrixXd K;//MatrixXd动态大小的矩阵类型
		Eigen::MatrixXd H_k;
		...
		for (int i = 0; i < iter_times; i++)
		{
			...
			Eigen::Matrix<double, 18, 18> J_inv;//雅可比逆矩阵
			J_inv.setIdentity();
			...
			Eigen::MatrixXd H_kt = H_kt.transpose();
			//R为0.001
